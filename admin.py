...
async def cmd_set_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # 1) Каждое блюдо с новой строки: "Суп - 300\nПлов - 500"
    # 2) Всё в одну строку: "Суп - 300 Плов - 500 Салат - 400"
    # Разбиваем по паттерну "цифры + пробел + заглавная/любая буква" — это начало нового блюда
    raw = raw.replace("\n", " ")
    # Вставляем разделитель перед каждым новым блюдом (после числа идёт пробел и текст)
    ...
    if not items:
        return await update.message.reply_text(
            ...
            "Формат (каждое блюдо с новой строки):\n"
            "/setmenu\nСуп - 300\nПлов - 500\nСалат - 400"
        )

    db.set_menu(items)

    lines_out = "\n".join(f"• {i['name']} — {i['price']} руб" for i in items)
...
# ── /confirm_payment (via callback) ───────────────────────────────────────────

async def cmd_confirm_payment(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Called from callback query when admin manually confirms"""
